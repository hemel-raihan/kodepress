            ...
            <table class="bordered" id="example">
                ...
                <tbody>
                    ...
                    <tr>
                        <td>{{ $key+1 }}</td>
                        <td>
                            <a href="{{route('posts.details',$blogcategorypost->id)}}">{{ \Illuminate\Support\Str::limit($blogcategorypost->title, 80, $end='...') }}</a>
                        </td>
                        <td>{{ $blogcategorypost->created_at->format('Y-m-d') }}</td>

                        <td>
                            ডাউনলোড:
                            <div>
                                <a href="{{ asset('uploads/contentfiles/'.$blogcategorypost->files) }}" title="Maternity Leave 696.pdf">
