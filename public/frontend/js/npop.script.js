...
function handleOpMobileChange(e) {
    ...
    if (e.matches) {
        ...
        document.getElementById("close_online_poll_view").style.cssText =
            "position:absolute;width:30px;height:30px;line-height:30px;text-align:center;left:-15px;top:15px;background-color:#fff;border-radius:50%;cursor:pointer;box-shadow:0 0 5px #000;z-index:5";
    } else {
        ...
        document.getElementById("close_online_poll_view").style.cssText =
            "position:absolute;width:30px;height:30px;line-height:30px;text-align:center;right:-15px;top:15px;background-color:#fff;border-radius:50%;cursor:pointer;box-shadow:0 0 5px #000;z-index:5";
    }
}
...
async function loadOpinionPoll(e, view_type = "") {
     ...
     */
    elm.innerHTML =
        '<span id="close_online_poll_view" onclick="closeOpinionPoll(this)">x</span><iframe src="' +
        url +
        "?req_from=" +
        __cur_location.host +
        '" frameborder=0 allowtransparency=yes scrolling=no width="100%" height="500" style="height:500px !important" />';
